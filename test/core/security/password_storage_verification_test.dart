...
void main() {
  group('Password Storage Verification', () {
    test('Password MUST be transformed into a hash before storage', () async {
      ...
      final storedHash = await PasswordHashingService.hashPassword(
        plainPassword,
      );
      ...
      expect(
        storedHash.contains(plainPassword),
        false,
      ...
      expect(
        storedHash.startsWith(RegExp(r'\$2[aby]\$')),
        true,
    ...
    test(
      'Two identical passwords MUST produce different hashes (Salting)',
      () async {
