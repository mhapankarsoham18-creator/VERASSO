      ...
      children: [
        ...
        const GlassContainer(
          child: Column(
            children: [
              ...
              SizedBox(height: 16),
              ShimmerLoading(child: SkeletonBox(width: 150, height: 28)),
              SizedBox(height: 8),
              ShimmerLoading(child: SkeletonBox(width: 100, height: 16)),
              SizedBox(height: 16),
              Row(
        ...
        GlassContainer(
          child: Column(
            ...
            children: [
              const ShimmerLoading(child: SkeletonBox(width: 60, height: 20)),
              const SizedBox(height: 12),
              const ShimmerLoading(
                child: SkeletonBox(width: double.infinity, height: 14),
              ),
              const SizedBox(height: 8),
              const ShimmerLoading(child: SkeletonBox(width: 200, height: 14)),
              const SizedBox(height: 16),
              const ShimmerLoading(child: SkeletonBox(width: 80, height: 20)),
              const SizedBox(height: 12),
              Wrap(
                spacing: 8,
...
class _StatSkeleton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const Column(
      children: [
        ShimmerLoading(child: SkeletonBox(width: 40, height: 24)),
        SizedBox(height: 4),
        ShimmerLoading(child: SkeletonBox(width: 60, height: 14)),
      ],
    );
  }
}
